...
class Warper:
    ...
    def forward_warp(self, frame1: torch.Tensor, mask1: torch.Tensor, depth1: torch.Tensor,
                     transformation1: torch.Tensor, transformation2: torch.Tensor, intrinsic: torch.Tensor) -> \
        ...
        :param frame1: (b, 3, h, w). If frame1 is not in the range [-1, 1], either set is_image=False when calling
                        ...
                        method accordingly.
        :param mask1: (b, 1, h, w) - 1 for known, 0 for unknown
        ...
        :param intrinsic: (b, 3, 3) camera intrinsic matrix
        """
    ...
    def compute_transformed_points(depth1: torch.Tensor, transformation1: torch.Tensor, transformation2: torch.Tensor,
        ...
        intrinsic_inv = torch.linalg.inv(intrinsic)  # (b, 3, 3)
        ...
        unnormalized_pos = torch.matmul(intrinsic_inv_4d, pos_vectors_homo)  # (b, h, w, 3, 1)
        ...
        trans_norm_points = torch.matmul(intrinsic_4d, trans_world)  # (b, h, w, 3, 1)
        return trans_norm_points

    def bilinear_splatting(self, frame1: torch.Tensor, mask1: torch.Tensor, depth1: torch.Tensor,
                           flow12: torch.Tensor, flow12_mask: torch.Tensor, is_image: bool = False) -> \
        ...
        :param flow12_mask: (b,1,h,w): 1 for valid flow, 0 for invalid flow
    ...
    def bilinear_interpolation(self, frame2: torch.Tensor, mask2: torch.Tensor, flow12: torch.Tensor,
                               flow12_mask: torch.Tensor, is_image: bool = False) -> Tuple[torch.Tensor, torch.Tensor]:
        """
...
def demo1():
    ...
    h, w = frame1.shape
    mask1 = torch.from_numpy(numpy.ones(shape=(h, w), dtype=bool))
